...
class CF(object):
    ...
    def mse(self, R ,M):
        ...
        # 计算预测值和实际值的均方误差
        return np.sum( ( (self.predict() - R) **2)) / M.sum()
...
#                                        np.dot(self.U.T, np.dot(np.diag(Wi), Y[:, i])))
        self.optimize_user(R,self.U,self.I,self.n_user,self.n_factor,self.lambd)
        self.optimize_item(R,self.U,self.I,self.n_item,self.n_factor,self.lambd)
